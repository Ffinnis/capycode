import { parseScopedThreadKey, scopeProjectRef, scopeThreadRef } from "@capycode/client-runtime";
import { type ScopedThreadRef, ThreadId } from "@capycode/contracts";
  ...
  const deleteThread = useCallback(
    async (
      ...
      opts: {
        ...
        skipWorktreeDeletePrompt?: boolean;
      } = {},
    ) => {
      const api = readEnvironmentApi(target.environmentId);
      if (!api) return;
