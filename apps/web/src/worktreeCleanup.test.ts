import { EnvironmentId, ProjectId, ThreadId } from "@capycode/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.t3/worktrees/capycode-mvp/capycode-4e609bb8",
    );
    expect(result).toBe("capycode-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.t3\\worktrees\\capycode-mvp\\capycode-4e609bb8",
    );
    expect(result).toBe("capycode-4e609bb8");
  });
