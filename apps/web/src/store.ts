...
import { createModelSelection, resolveModelSlugForProvider } from "@capycode/shared/model";
...
export interface EnvironmentState {
  ...
  activeWorkspaceIdByProjectId: Record<ProjectId, WorkspaceId | null>;
  workspaceSectionIdsByProjectId: Record<ProjectId, WorkspaceSectionId[]>;
  workspaceSectionById: Record<WorkspaceSectionId, WorkspaceSection>;
  threadIds: ThreadId[];
...
function mapWorkspace(
  ...
  return {
    ...
    deletingAt: workspace.deletingAt,
  };
}

function mapWorkspaceSection(
  workspaceSection: OrchestrationReadModel["workspaceSections"][number],
  environmentId: EnvironmentId,
): WorkspaceSection {
  return {
    id: workspaceSection.id,
    environmentId,
    projectId: workspaceSection.projectId,
    name: workspaceSection.name,
    tabOrder: workspaceSection.tabOrder,
    isCollapsed: workspaceSection.isCollapsed,
    color: workspaceSection.color,
    createdAt: workspaceSection.createdAt,
  };
}
...
> {
  ...
  const workspaceById: Record<WorkspaceId, Workspace> = {};
  const workspaceIdsByProjectId: Record<ProjectId, WorkspaceId[]> = {};
  const activeWorkspaceIdByProjectId: Record<ProjectId, WorkspaceId | null> = {};
  const workspaceSectionIdsByProjectId: Record<ProjectId, WorkspaceSectionId[]> = {};
  const workspaceSectionById: Record<WorkspaceSectionId, WorkspaceSection> = {};
  ...
  for (const workspace of workspaces) {
    ...
    if (workspace.isActive) {
      activeWorkspaceIdByProjectId[workspace.projectId] = workspace.id;
    } else if (!(workspace.projectId in activeWorkspaceIdByProjectId)) {
      activeWorkspaceIdByProjectId[workspace.projectId] = null;
    }
  }

  for (const workspaceSection of workspaceSections) {
    workspaceSectionIdsByProjectId[workspaceSection.projectId] = [
      ...(workspaceSectionIdsByProjectId[workspaceSection.projectId] ?? []),
      workspaceSection.id,
    ];
    workspaceSectionById[workspaceSection.id] = workspaceSection;
  }
...
function syncEnvironmentReadModel(
  ...
  const workspaces = readModel.workspaces
    .filter((workspace) => workspace.deletingAt === null)
    .map((workspace) => mapWorkspace(workspace, environmentId));
  const workspaceSections = readModel.workspaceSections.map((workspaceSection) =>
    mapWorkspaceSection(workspaceSection, environmentId),
  );
...
export function selectWorkspacesForProjectRef(
  ...
  const workspaceIds = environmentState.workspaceIdsByProjectId[ref.projectId] ?? [];
...
export function selectWorkspaceSectionsForProjectRef(
  state: AppState,
  ref: ScopedProjectRef | null | undefined,
): WorkspaceSection[] {
  if (!ref) {
  ...
  const environmentState = selectEnvironmentState(state, ref.environmentId);
  const workspaceSectionIds = environmentState.workspaceSectionIdsByProjectId[ref.projectId] ?? [];
  return workspaceSectionIds.flatMap((workspaceSectionId) => {
    const workspaceSection = environmentState.workspaceSectionById[workspaceSectionId];
    return workspaceSection ? [workspaceSection] : [];
  });
}

export function selectActiveWorkspaceForProjectRef(
  state: AppState,
  ref: ScopedProjectRef | null | undefined,
): Workspace | undefined {
  if (!ref) {
  ...
  const environmentState = selectEnvironmentState(state, ref.environmentId);
  const activeWorkspaceId = environmentState.activeWorkspaceIdByProjectId[ref.projectId] ?? null;
  return activeWorkspaceId ? environmentState.workspaceById[activeWorkspaceId] : undefined;
}
...
export function selectWorkspacesAcrossEnvironments(state: AppState): Workspace[] {
  return getEnvironmentEntries(state).flatMap(([, environmentState]) =>
    environmentState.workspaceIds.flatMap((workspaceId) => {
      const workspace = environmentState.workspaceById[workspaceId];
      return workspace ? [workspace] : [];
    }),
  );
