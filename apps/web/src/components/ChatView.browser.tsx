...
describe("ChatView timeline estimator parity (full app)", () => {
  ...
  it("falls back to the first installed editor when the stored favorite is unavailable", async () => {
    localStorage.setItem("capycode:last-editor", JSON.stringify("vscodium"));
    setDraftThreadWithoutWorktree();
  ...
  it("does not bootstrap worktree creation for legacy worktree-mode drafts", async () => {
    ...
    try {
      ...
      const sendButton = await waitForElement(
        ...
        "Unable to find draft composer submit button.",
      );
  ...
  it("hides the archive action when the pointer leaves a thread row", async () => {
    ...
    try {
      ...
      expect(document.querySelector(`[data-testid="thread-delete-${THREAD_ID}"]`)).toBeNull();
  ...
  it("shows the confirm archive action after clicking the archive button", async () => {
    ...
    try {
      ...
      localStorage.removeItem("capycode:client-settings:v1");
      await mounted.cleanup();
    }
  });

  it("does not render a delete action for thread rows", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
        targetMessageId: "msg-user-no-delete-thread-test" as MessageId,
        targetText: "no delete thread target",
      }),
    });
  ...
  it("keeps the active draft route mounted after the first send bootstrap", async () => {
    const mounted = await mountChatView({
      viewport: DEFAULT_VIEWPORT,
      snapshot: createSnapshotForTargetUser({
    ...
    try {
      ...
      useComposerDraftStore.getState().setPrompt(newDraftId, "first bootstrap turn");
      await waitForLayout();
      ...
      await vi.waitFor(
        () => {
          const threadTurnRequests = wsRequests.filter(
            (request) =>
          ...
          expect(threadTurnRequests[0]).toMatchObject({
