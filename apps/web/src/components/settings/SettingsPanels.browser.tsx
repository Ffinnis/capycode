...
const createDesktopBridgeStub = (overrides?: {
  ...
  return {
    ...
    stopNotificationSoundPreview: vi.fn().mockResolvedValue(undefined),
    importCustomNotificationSound: vi.fn().mockResolvedValue({ canceled: true, sound: null }),
  };
};
...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(page.getByText("Capycode will restart to expose this environment over the network."))
      .toBeInTheDocument();
