...
import { parsePersistedServerObservabilitySettings } from "@capycode/shared/serverSettings";
import {
  AuthSessionId,
  CommandId,
...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to CAPYCODE_HOME)."),
  Flag.optional,
);
...
const EnvServerConfig = Config.all({
  ...
  t3Home: Config.string("CAPYCODE_HOME").pipe(
    Config.option,
    Config.map((value) => Option.getOrUndefined(value) ?? process.env.T3CODE_HOME),
  ),
...
const serveCommand = Command.make("serve", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription(
    "Run the Capycode server without opening a browser and print headless pairing details.",
  ),
  Command.withHandler((flags) =>
