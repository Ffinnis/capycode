  ...
  function* () {
    ...
    const resolveWorkspaceContextForThread = Effect.fn("resolveWorkspaceContextForThread")(
      function* (input: {
        ...
        readonly explicitWorkspaceId?: string | null;
      ...
      > {
        if (input.explicitWorkspaceId) {
          const explicitWorkspaceRows = yield* sql<{
            ...
            WHERE
              workspaces.id = ${input.explicitWorkspaceId}
              AND workspaces.project_id = ${input.projectId}
              AND workspaces.deleting_at IS NULL
            LIMIT 1
          `.pipe(
            Effect.mapError(
              toPersistenceSqlError(
                "ProjectionPipeline.resolveWorkspaceContextForThread:findExplicitWorkspace",
              ),
            ),
          );

          const explicitWorkspace = explicitWorkspaceRows[0];
          if (explicitWorkspace) {
            ...
            `.pipe(
              Effect.mapError(
                toPersistenceSqlError(
                  "ProjectionPipeline.resolveWorkspaceContextForThread:seedActiveWorkspace",
                ),
              ),
            );
        ...
        if (input.worktreePath !== null) {
          const existingWorktreeRows = yield* sql<{
            ...
            SELECT
              ...
              path
            FROM worktrees
            WHERE project_id = ${input.projectId} AND path = ${input.worktreePath}
            ORDER BY created_at ASC, id ASC
            LIMIT 1
          `.pipe(
            Effect.mapError(
              toPersistenceSqlError(
                "ProjectionPipeline.resolveWorkspaceContextForThread:findWorktree",
              ),
            ),
          );

          const existingWorktree = existingWorktreeRows[0] ?? null;
          const worktreeId = existingWorktree?.id ?? crypto.randomUUID();
          if (existingWorktree === null) {
            ...
            `.pipe(
              Effect.mapError(
                toPersistenceSqlError(
                  "ProjectionPipeline.resolveWorkspaceContextForThread:createWorktree",
                ),
              ),
            );
            resolvedBranch = input.branch ?? "workspace";
            resolvedWorktreePath = input.worktreePath;
          } else {
            resolvedBranch = existingWorktree.branch;
            resolvedWorktreePath = existingWorktree.path;
          }
          ...
          `.pipe(
            Effect.mapError(
              toPersistenceSqlError(
                "ProjectionPipeline.resolveWorkspaceContextForThread:findWorktreeWorkspace",
              ),
            ),
          );
          ...
          if (resolvedWorkspaceId === null) {
            ...
            yield* sql`
              ...
              VALUES (
                ...
                'worktree',
                ${resolvedBranch},
                ${resolvedBranch ?? resolvedWorktreePath?.split(/[\\/]/).pop() ?? "workspace"},
                ${nextTabOrder},
                0,
                ${input.createdAt},
                ${input.updatedAt},
            ...
            `.pipe(
              Effect.mapError(
                toPersistenceSqlError(
                  "ProjectionPipeline.resolveWorkspaceContextForThread:createWorktreeWorkspace",
                ),
              ),
            );
          } else if (existingWorkspaceRows[0]?.branch !== resolvedBranch) {
            yield* sql`
              UPDATE workspaces
              SET
                branch = ${resolvedBranch},
                updated_at = ${input.updatedAt},
                last_opened_at = ${input.updatedAt}
              WHERE id = ${resolvedWorkspaceId}
            `.pipe(
              Effect.mapError(
                toPersistenceSqlError(
                  "ProjectionPipeline.resolveWorkspaceContextForThread:updateWorktreeWorkspace",
                ),
              ),
            );
          }
        ...
        `.pipe(
          Effect.mapError(
            toPersistenceSqlError(
              "ProjectionPipeline.resolveWorkspaceContextForThread:ensureProjectState",
            ),
          ),
        );
    ...
    const applyThreadsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        case "thread.created":
          const createdWorkspaceContext = yield* resolveWorkspaceContextForThread({
            projectId: event.payload.projectId,
          ...
          yield* projectionThreadRepository.upsert({
            ...
            projectId: event.payload.projectId,
            workspaceId: createdWorkspaceContext.workspaceId as any,
            title: event.payload.title,
            modelSelection: event.payload.modelSelection,
            runtimeMode: event.payload.runtimeMode,
            interactionMode: event.payload.interactionMode,
            branch: createdWorkspaceContext.branch,
            worktreePath: createdWorkspaceContext.worktreePath,
            latestTurnId: null,
            createdAt: event.payload.createdAt,
            updatedAt: event.payload.updatedAt,
            archivedAt: null,
        ...
        case "thread.meta-updated": {
          ...
          yield* projectionThreadRepository.upsert({
            ...existingRow.value,
            workspaceId: updatedWorkspaceContext.workspaceId as any,
            ...(event.payload.title !== undefined ? { title: event.payload.title } : {}),
            ...
            event.payload.worktreePath !== undefined
              ? {
                  branch: updatedWorkspaceContext.branch,
                  worktreePath: updatedWorkspaceContext.worktreePath,
                }
              : {}),
            updatedAt: event.payload.updatedAt,
          });
